...
import com.jazzkuh.commandlib.annotations.Completion;
...
public abstract class AnnotationExtendedCommand {
    ...
    public boolean execute(CommandSender sender, String[] args, boolean isMainCommand) {
        ...
        int size = parameters.stream().filter(parameter -> !parameter.isAnnotationPresent(com.jazzkuh.commandlib.annotations.Optional.class)).toList().size();
        ...
        for (int i = 1; i < parameters.size(); i++) {
            ...
            if (args.length <= (isMainCommand ? i - 1 : i) && parameter.isAnnotationPresent(com.jazzkuh.commandlib.annotations.Optional.class)) {
